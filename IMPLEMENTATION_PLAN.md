...
### Phase 2 — Extension: Scan _(~1 week)_
- WXT scaffold; side panel with sign-in; optional host permission request on first Scan.
- Cookie check → candidates → background-tab account-page snapshot → `/agent/classify` → list +
  estimate.
- **Exit:** install unpacked → Scan → your real signed-in subscriptions with plan/price/estimate.
...
## 11. Open items
- WXT vs Plasmo; first 5 merchants to seed (from your own signed-in subscriptions); fee term
  (offer term vs first month); brand name.
