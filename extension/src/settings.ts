...
export interface Settings { apiBase: string; clientKey: string; testMode: boolean; testDomain: string; testAccountUrl: string; testName: string; watch: boolean; maxSteps: number; skipPayment: boolean; maxHunts: number }
// Set WXT_API_BASE in extension/.env (e.g. https://your-site.netlify.app) before `npm run package:extension`.
export const DEFAULTS: Settings = { apiBase: (import.meta as any).env?.WXT_API_BASE || '', clientKey: '', testMode: false, testDomain: '', testAccountUrl: '', testName: 'Test service', watch: false, maxSteps: 25, skipPayment: true, maxHunts: 10 };
