...
// charges 10% of what was actually verified, so a run that saves less than estimated charges less,
...
export const feeCentsFor = (savingsUsd) => (savingsUsd > 0 ? Math.max(100, Math.round(savingsUsd * 10)) : 0);   // 10%, $1 minimum, $0 if nothing
...
export default async (req) => handle(req, async (body) => {
  ...
  const session = await stripe.checkout.sessions.create({
    ...
    custom_text: { submit: { message: `Nothing is charged now. After the run you pay 10% of what we actually saved you: about $${estFee.toFixed(2)} if every offer comes through, less if some don't, $0 if none.` } },
