// After the run: one charge of 15% of VERIFIED savings to the card saved at checkout, or
...
import { feeCentsFor, STRIPE_MIN_CENTS } from './checkout.mjs';
...
export default async (req) => handle(req, async (body) => {
  ...
  // No minimum fee, but Stripe cannot charge below 50¢. Such a fee is waived rather than attempted and failed.
  if (feeCents < STRIPE_MIN_CENTS) return { feeCents, estimatedFeeCents, adjusted, charged: false, waived: true };

  try {
    const pi = await stripe.paymentIntents.create({
      amount: feeCents, currency: 'usd', customer: customerId, payment_method: paymentMethod, off_session: true, confirm: true,
      description: `Walkaway — 15% of verified savings ($${verified.toFixed(2)})`,
